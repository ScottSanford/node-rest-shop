...
### Overview
The Node Rest Shop API is a REST API that can be used to interactive with products and orders that are stored in a MongoDB as well as manage accounts. All the responses are JSON and input is provided as JSON objects in the BODY of the requests.  

Some calls are protected and require a JSON Web Token (JWT) to indicate the user is authorized to read or write data to the database. If an endpoint requires a JWT, it will be listed with...
Headers: `Authorization: Bearer <JWT>`

----

### Version
The current version of the API is v1. Please reference the `v1` in each endpoint request. 
...
Description: Logins a user and returns a JWT for authentication.
